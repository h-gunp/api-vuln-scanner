...
백엔드의 REST API, Job·DB·Artifact 저장과 프론트엔드 DTO는 LLM 구현 범위가 아님.

Executor의 HTTP 요청, Evidence 수집, 규칙 판정도 LLM 구현 범위가 아님.
...
| LLM API endpoint를 LLM 책임으로 기재 | 백엔드 명세와 책임 중복 | LLM은 내부 서비스 메서드만 제공 |
...
별도 FastAPI 서버, DB 모델, 메시지 큐 SDK, 에이전트 프레임워크와 PDF 생성기는
...
1. 규칙 기반으로 후보와 binding hint를 생성함.
...
| 단계 | 버전 | SHA-256 |
| --- | --- | --- |
| L1 | `rel-v2` | `846c4a3eaa6354d9b8994c7cbce1e3a6c65e148248513bfd7e575baed2963baa` |
...
백엔드 작업 핸들러는 다음 순서로 호출함.

```
relationship = service.analyze_relationships(target_profile, api_graph)
plan = service.create_scan_plan(
    target_profile,
    api_graph,
    relationship,
    requests_already_used=current_request_count,
)
report = service.create_ai_report(scan_result)
```

백엔드는 각 Pydantic 결과를 `model_dump(mode="json")`으로 직렬화하고 기존

Artifact 저장·체크섬·Job 상태 처리에 연결함.
...
- API 호출 없는 단위 테스트와 실제 API 키를 사용한 통합 테스트를 구분함.